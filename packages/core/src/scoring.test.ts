...
import type { JdDetail, Profile } from './types';
...
describe('scoreJob', () => {
  it('shortlists a strong senior-frontend match', () => {
    const jd: JdDetail = {
    ...
    const r = scoreJob(jd, profile, { easyApply: true, companyRating: 4.1 });
    expect(r.skillMatchPct).toBeGreaterThan(70);
    expect(r.experienceMatchPct).toBe(100);
    expect(r.passedGate).toBe(true);
    expect(r.priorityScore).toBeGreaterThan(70);
  });

  it('rejects a pure-backend role via roleFit gate', () => {
    const jd: JdDetail = {
    ...
    const r = scoreJob(jd, profile, {});
    expect(r.roleFitPct).toBe(0);
    expect(r.passedGate).toBe(false);
  });

  it('penalizes a service company', () => {
    const jd: JdDetail = {
    ...
    const r = scoreJob(jd, profile, { companyRating: 3.0 });
    expect(r.companyFitPct).toBeLessThan(40);
  });
});
